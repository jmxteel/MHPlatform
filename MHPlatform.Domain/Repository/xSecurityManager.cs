...
namespace MHPlatform.Domain.Repository
{
    public class xSecurityManager
    {
        ...
        private readonly UserAuthBase _auth;

        public xSecurityManager(InstallationContext context, UserAuthBase auth)
        {
            _context = context;
            _auth = auth;
        }
        ...
        public UserAuthBase ValidateUser(string userName, string password)
        {
            ...
            try
            {
                list = _context.Users!.Where(u => u.UserName.ToLower() == userName.ToLower() &&
                u.Password.ToLower() == password.ToLower()).ToList();

                if(list.Count() > 0)
                {
                    UserAuthBase _auth = BuildUserAuthObject(list[0].UserId, userName);
                }
            } catch (Exception ex)
            {
